...
import { getNpcSpriteForType } from './player/npc_sprite_picker.js';
...
export function createNpcByType(type, id, x, y, options = {}) {
  ...
  if (Ctor === PlayerWorkerNpc) {
    return new PlayerWorkerNpc(id, x, y, options);
  }
  const withSprite = {
    ...options,
    sprite: options?.sprite || getNpcSpriteForType(type)
  };
  return new Ctor(id, x, y, withSprite);
}
